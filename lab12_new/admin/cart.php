...
if (isset($_POST['id']) && isset($_POST['title']) && isset($_POST['description']) && isset($_POST['price']) && isset($_POST['weight']) && isset($_POST['photo'])) {
    ...
    for ($i = 1; $i <= $_SESSION['count']; $i++) {
        if ($_SESSION[$i.'_1'] == $id) {
            $_SESSION[$i.'_2']++; // Increment quantity if product already exists
            header('Location: cart.php');
            exit(); // Add this to stop further execution after redirect
        }
    }
...
function ShowCart() {
    global $full_price;
    global $full_weight;
    $wynik = '<a href="shop.php">Powrót</a><table><tr><th>Zdjęcie</th><th>Tytuł</th><th>Opis</th><th>Waga</th><th>Cena</th><th>Sztuk</th><th>Data</th><th>Usuń</th></tr>';
    ...
    $wynik .= '<button type="submit" style="width: 100px; height: 30px; background-color: red; outline: none; border: none; cursor: pointer;">Zamów</button>';
    $wynik .= '</form>';
    echo $wynik;
}
...
function ShowProduct($nr) {
    ...
    $wynik .= '<td><form action="'.$_SERVER['REQUEST_URI'].'" method="POST"><button style="border: none; outline: none; cursor:pointer; font-size:1em" type="submit" name="usun" value="'.$_SESSION[$nr.'_0'].'">Usun</button></form></td></tr>';
...
function Order() {
        session_destroy();
        echo "Pomyślnie złożono zamówienie";
